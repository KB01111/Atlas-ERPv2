...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    const totalStorageBytes = storageUsage.data?.reduce((sum, doc) => sum + (doc.file_size || 0), 0) || 0;
    ...
    const documentGrowth = lastMonthDocs > 0 ? ((currentMonthDocs - lastMonthDocs) / lastMonthDocs * 100) : 0;
